	...
	public static void LoopAB(WebDriver driver,String url) {
	    ...
	    try {
		    ...
		    for (WebElement detailSolutionRows : solutionRows) {
		    	...
		    	if (language.length() == 0 && ( detailSolution.get(2).getText().contains("Java")|| detailSolution.get(2).getText().contains("JS") )) {
	 ...
	 public static void Login(WebDriver driver) {
			...
			try {
				driver.get(loginUrl);
		        Thread.currentThread().sleep(2000);
		        driver.findElement(By.name("username")).sendKeys("AAAAAAA");;
		...
		try {
			...
			if (menthodName.length() > 0) {
				...
				//menthodName = menthodName.replace("function ", "");
			     String folderJS ="F:\\Selenium\\SourceNew\\JS\\";
			     String folderJava = "F:\\Selenium\\SourceNew\\Java\\";
